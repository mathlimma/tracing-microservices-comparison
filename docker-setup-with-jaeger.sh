...
./mvnw clean install
docker build -t com.example/animal-name-service:0.1.0 -f Dockerfile_Jaeger .
...
./mvnw clean install
docker build -t com.example/name-generator-service:0.1.0 -f Dockerfile_Jaeger .
...
./mvnw clean install
docker build -t com.example/scientist-name-service:0.1.0 -f Dockerfile_Jaeger .
